...
function getFileNameFromTraits(traits: PotrTraits) {
    ...
    const traitIdxs = TRAIT_TYPES.map((traitType) => getTraitIndex(baseClassName, traitType, traits[traitType]));
...
function getTraitIndex(baseClassName, traitType, trait) {
    ...
    const { traits } = isClassDependentTraitType(traitType) ? TRAIT_WEIGHTS[traitType][baseClassName] : TRAIT_WEIGHTS[traitType];
    return (traits as string[]).indexOf(trait);
}

const isHumanoid = (className: string) => !["Dragon", "Golem", "Phantom"].includes(className);
const isClassDependentTraitType = (traitType: TraitType) => CLASS_DEPENDENT_TRAIT_TYPES.includes(traitType);
